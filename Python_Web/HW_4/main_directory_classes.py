...
class MainDirectory:
    ...
    def scan_directory(self):
        ...
        with ThreadPoolExecutor() as executor:
            executor.map(self.scanner.scan, dir_paths)
...
class FilesSorter:
    ...
    def sort(self):
        Thread(target=self.sort_images).start()
        Thread(target=self.sort_videos).start()
        Thread(target=self.sort_audio).start()
        Thread(target=self.sort_docs).start()
        Thread(target=self.sort_archives).start()
        Thread(target=self.sort_unknown).start()
