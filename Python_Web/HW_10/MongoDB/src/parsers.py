...
class TopicParser(Parser):
    ...
    def create_parser(self):
        cyril_symbols = ''.join([chr(i) for i in range(1040, 1104)] + [chr(1108), chr(1110), chr(1111)])
        raw_symbols = Word(printables + ' ', excludeChars=alphanums).leaveWhitespace()
...
class TagParser(Parser):
    ...
    def create_string(self, items: list):
        basic_str = items[0]
        for count, value in enumerate(items[1:]):
            if value[0] in '-_' or basic_str[-1] in '-_':
                basic_str += value
            else:
                basic_str += (' ' + value)
        return basic_str
    ...
    def parse_string(self) -> str:
        ...
        tag = self.create_string([t.strip() for t in tag if t[0] != ' '])
...
class TextParser(Parser):
    ...
    def create_parser(self):
        ...
        sentence_parser = OneOrMore(Word(alphanums) + Optional(non_end_symbols)) + Optional(end_symbols)
        text_parser = ZeroOrMore(sentence_parser)
        return text_parser

    def create_string(self, text_list: list, ignore_case: bool) -> str:
        sentence = []
        sentences = []
        text = []
        ...
        # if last sentence has no stop mark, we also add this sentence to list of sentences
        if sentence:
            sentences.append(sentence)
        ...
        for sentence in sentences:
            ...
            if ignore_case:
                ...
                text.append(sentence_str.replace(sentence_str[0], sentence_str[0].upper(), 1))
