...
from data import FileDuplicatesDict
...
class File:
    ...
    def __init__(self, basepath, norm_path, maindir_path):
        self.filepath = AsyncPath(basepath)
        self.basepath = AsyncPath(maindir_path)
    ...
    async def move_file(self):
        self.filepath = await self.filepath.rename(self.basepath / self.filename)
    # def move_file(self):
