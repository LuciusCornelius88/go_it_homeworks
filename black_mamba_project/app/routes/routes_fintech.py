...
def get_all_headers():
	headers = db.session.query(FintechNews).all()
...
def sort_by_subject():
	...
	headers = db.session.query(FintechNews).filter_by(category=subject).order_by(desc(FintechNews.published_on)).all()
	return render_template('fintech_templates/fintech_output.html', headers=headers)
...
def sort_by_language():
	...
	headers = db.session.query(FintechNews).filter_by(language=lang).order_by(desc(FintechNews.published_on)).all()
	return render_template('fintech_templates/fintech_output.html', headers=headers)
...
def order_by_date():
	headers = db.session.query(FintechNews).order_by(desc(FintechNews.published_on)).all()
	return render_template('fintech_templates/fintech_output.html', headers=headers)
...
def fintech_delete_all():
	db.session.query(FintechNews).delete()
	db.session.commit()
